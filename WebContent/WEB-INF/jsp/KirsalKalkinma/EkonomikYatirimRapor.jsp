	...
	<div class="container-fluid" id="yazdir">
		<div class="container">
			<div id="container" class="col-md-10">
					...
					<tbody class="govde">
						...
						<c:forEach items="${tumEkonomikYatirimListesi }" var="yatirim"
							...
							<c:if test="${yatirim.etapNo == 11 }">
								...
								<c:set var="kapasite10" value="${kapasite10 + yatirim.kapasite}" />
								<c:set var="istihdam10" value="${istihdam10 + yatirim.istihdam}" />
						...
						<c:forEach items="${tumEkonomikYatirimListesi }" var="yatirim"
							...
							<c:if test="${yatirim.etapNo == 12 }">
								...
								<c:set var="kapasite11" value="${kapasite11 + yatirim.kapasite}" />
								<c:set var="istihdam11" value="${istihdam11 + yatirim.istihdam}" />
						...
						<c:forEach items="${tumEkonomikYatirimListesi }" var="yatirim"
							...
							<c:if test="${yatirim.etapNo == 13 }">
								...
								<c:set var="kapasite12" value="${kapasite12 + yatirim.kapasite}" />
								<c:set var="istihdam12" value="${istihdam12 + yatirim.istihdam}" />
						...
						<c:if test="${etapNo13}">
							<tr>
								...
								<td><fmt:formatNumber pattern="#,##0.00 TL" type="currency"
										value="${hibeTutari12}" var="hibeTutari12"></fmt:formatNumber>${hibeTutari12}</td>
								<td><fmt:formatNumber pattern="#,##0" type="number"
				...
				<table class="table table-striped bg-danger" border="1"
					id="yillaraGore">
					<thead>
						<tr>
							<td align="center" colspan="13">YILLARA GÖRE İLÇE DAĞILIMI</td>
						</tr>


						<tr>
							<td rowspan="2"
								style="width: auto; height: 15px; line-height: 15px; top: 50%;">İLÇE</td>
							<td colspan="3">${etapNo }</td>
						...
						<c:set var="adetSarıçam" value="0"></c:set>
						<c:set var="adetYüreğir" value="0"></c:set>
						<c:forEach items="${tumEkonomikYatirimListesi }" var="yatirim"
							varStatus="sira">
							<c:if test="${yatirim.etapNo eq 3 }">

								<c:if test="${yatirim.ilce.isim eq 'Sarıçam'}">
									<c:set var="adetSarıçam" value="${adetSarıçam+1 }"></c:set>
								</c:if>
								<c:if test="${yatirim.ilce.isim eq 'Yüreğir'}">
									<c:set var="adetYüreğir" value="${adetYüreğir+1 }"></c:set>
								</c:if>
							</c:if>
						</c:forEach>
						...
						<c:forEach items="${tumEkonomikYatirimListesi }" var="yatirim"
							...
							<c:if test="${yatirim.etapNo eq 3 }">
								<tr>
									...
									<td>${yatirim.projeBedeli}</td>
									<td>${yatirim.hibeTutari}</td>
								</tr>
